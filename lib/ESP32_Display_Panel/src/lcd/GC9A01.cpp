...
static esp_err_t panel_gc9a01_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end, const void *color_data)
{
    ...
    // Kiểm tra thứ tự tọa độ
    assert(x_start < x_end && y_start < y_end);

    uint8_t x_coord[4];
    x_coord[0] = static_cast<uint8_t>((static_cast<unsigned>(x_start) >> 8) & 0xFF);
    x_coord[1] = static_cast<uint8_t>(static_cast<unsigned>(x_start) & 0xFF);
    x_coord[2] = static_cast<uint8_t>((static_cast<unsigned>(x_end - 1) >> 8) & 0xFF);
    x_coord[3] = static_cast<uint8_t>(static_cast<unsigned>(x_end - 1) & 0xFF);

    uint8_t y_coord[4];
    y_coord[0] = static_cast<uint8_t>((static_cast<unsigned>(y_start) >> 8) & 0xFF);
    y_coord[1] = static_cast<uint8_t>(static_cast<unsigned>(y_start) & 0xFF);
    y_coord[2] = static_cast<uint8_t>((static_cast<unsigned>(y_end - 1) >> 8) & 0xFF);
    y_coord[3] = static_cast<uint8_t>(static_cast<unsigned>(y_end - 1) & 0xFF);
